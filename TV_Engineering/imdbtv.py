...
class DoubanTV(threading.Thread):
	...
	def all_return_body(self):
		...
		sel2 = Selector(text=urllib2.urlopen(ur2).read())
		#制作公司和发行公司
		url3 = 'http://www.imdb.com/title/{0}/companycredits'.format(ttid,)
		sel3 = Selector(text=urllib2.urlopen(ur3).read())
		#职员
	...
	def imdb_info(self, url, sel):
		...
		#基本信息
		self.info_dict['Relate_ID'] = relate_id
		self.info_dict['source'] = 'IMDB'
		self.info_dict['movie_url'] = movie_url
		self.info_dict['movie_id'] = ttid
		#0图片
		if sel.xpath('//*[@class="poster"]//*[@itemprop="image"]/@src').extract():
			...
			img_response = urllib2.urlopen(self.info_dict['img_url']).read()
			self.info_dict['img_data'] = bson.Binary(img_response) #影片图片数据
		else:
			self.info_dict['img_url'] = ''
			self.info_dict['img_data'] = ''
		#1影片名称 2外文名称
...
def get_info(sel):
	...
	if is_detail:
		...
		self.info_dict['language'] = language_list
		self.info_dict['runtime'] = runtime
	return info_dict
...
def release_info(ttid):
	...
	self.info_dict['release_info'] = release_list
	self.info_dict['other_name'] = othername_list
	return info_dict
...
def company_info(ttid):
	...
	self.info_dict['production_company'] = production_company
	self.info_dict['distributors'] = distributors_list
	return info_dict
#职员
def cast_info(ttid):
