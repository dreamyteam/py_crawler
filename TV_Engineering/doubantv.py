...
class DoubanTV(object):
	...
	def __init__(self, movie_dict, flag):
		...
		self.flag = flag
		self.history_dict = history_formate()

	def run(self):
		
		if self.flag == 'first':
			sel = self.return_info()
			base_data = self.base_info(sel)
			if base_data != 'pass':
				self.info_dict.update(base_data)
	...
	def update_info(self, sel):
		...
		if sel.xpath('//*[@property="v:average"]'):
			...
			if self.info_dict['average']:
				...
				self.history_dict['average'] = self.info_dict['average']
		...
		if sel.xpath('//*[@property="v:votes"]'):
			...
			self.history_dict['votes'] = self.info_dict['votes']
		...
		if sel.xpath('//*[@href="https://movie.douban.com/subject/{0}/comments"]'.format(movie_id,)):
			...
			self.history_dict['all_short'] = self.info_dict['all_short']
		...
		if sel.xpath('//*[@href="https://movie.douban.com/subject/{0}/reviews"]'.format(movie_id,)):
			...
			self.history_dict['all_long'] = self.info_dict['all_long']
			print u'长评数量:%s' % self.info_dict['all_long']

		self.info_dict['update_time'] = time.strftime('%Y-%m-%d %H:%M:%S')
		self.history_dict['update_time'] = self.info_dict['update_time']
		print u'更新时间:%s' % self.info_dict['update_time']

		self.info_dict['history_info'].append(self.history_dict)
...
def run_threads():
	...
	count = 0
	data = db.DoubanTagID.find().skip(4300).limit(700)
	for i in data:
		count += 1
		print u'第几个:%s' % count
		time.sleep(2)
