...
class Dmzj(object):
	...
	def base_info_2(self):
		...
		res = urllib2.urlopen('http://www.dmzj.com/static/hits/{0}.json'.format(self.movie_dict['js_id']))
		json_data = json.loads(res.read())
		if 'hot_hits' in json_data:
			self.info_dict['popular'] = json_data['hot_hits']
		# print u'人气:%s' % self.info_dict['popular']
...
def run_threads():
	...
	mongo_data = db.CartoonSource.find({'source':'dmzj','url':{'$regex':'http://manhua.'}}).sort({'_id': -1})
